...
└── Courses/
    └── ICT211 — Database Design/
        ...
        ├── Assignments/
        └── Exam Prep/
See examples/vault-structure/example-vault-tree.md for the full tree.

Requirements

Python 3.11+
An Obsidian installation (free) — obsidian.md
A Canvas LMS account with API access
An Anthropic API key (scripts 4 and 5 only) — console.anthropic.com
...
Select ~/Desktop/University
Install the Dataview and Obsidian Spaced Repetition plugins (community plugins)
...
Navigate to each course → Files (or the lecture page with "Downloadable slides")
Download the PDFs/PPTXs
Sort them into subfolders by course code:
~/Desktop/slides/
├── ICT200/
│   ├── Week1_SDLC.pdf
